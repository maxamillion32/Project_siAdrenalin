...
public class Stevec extends View{
    ...
    //Za vklop ozadja odkomentiraj vse kar ima veze z nMask!
    //private Bitmap mMask;

	public Stevec(Context context) {
		super(context);
		init();
	...
	public Stevec(Context context, AttributeSet attrs)
	{
		super(context, attrs);
        ...
        try {
            // read attributes
            setMaxSpeed(attributes.getFloat(R.styleable.Stevec_maxSpeed, (float) DEFAULT_MAX_SPEED));
            setSpeed(attributes.getFloat(R.styleable.Stevec_speed, 0));
            setLabelTextSize(attributes.getDimensionPixelSize(R.styleable.Stevec_labelTextSize, Math.round(DEFAULT_LABEL_TEXT_SIZE_DP * density)));
        } finally {
            attributes.recycle();
        }
        init();
	...
	public Stevec(Context context, AttributeSet ats, int defaultStyle) {
	...
	public double getMaxSpeed() {
	 ...
	 public void setMajorTickStep(double majorTickStep) {
	    ...
	    private void init() {
	        ...
	        //mMask = BitmapFactory.decodeResource(getResources(), R.drawable.spot_mask2);
